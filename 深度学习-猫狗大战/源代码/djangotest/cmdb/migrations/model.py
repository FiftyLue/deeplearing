...
'''
卷积神经网络模型的构造，
包含两个卷积层（每个卷积层后加一个池化层），
两个全连接层，
最后一个softmax输出分类结果
'''
#训练时需要用到的方法
def inference_train(images, batch_size, n_classes):
...
def losses_train(logits, labels):
...
def trainning_train(loss, learning_rate):
...
def evaluation_train(logits, labels):
